...
Respuesta: El número de hijos parece tener un impacto negativo significativo en el ingreso anual. Las variables de género, edad y educación universitaria no presentan efectos significativos en este conjunto de datos, ya que sus valores p son altos. Por cada hijo adicional, el ingreso anual disminuye en promedio en \$13,400.18. El intercept (ingreso anual si el resto de las variables son 0) indica un gran nivel de significancia. Esto en un contexto real puede no tener sentido, sobre todo teniendo en cuenta que una de las variables X utilizadas es la edad.
...
**Ejercicio 1:** Interprete el modelo. Que significan los coeficientes obtenidos? Solo la educación primaria tiene un efecto significativo de votar a Trump. Intercepto: logaritmo de la probabilidad de que una persona vote por Trump cuando ingreso es 0, el género es "Femenino" y el nivel educativo es Doctorado

**Ejercicio 2:** convierta los coeficientes en OR, como los interpreta? Las personas con educación primaria tienen 2.72 veces más probabilidades de votar por Trump en comparación con la categoría de referencia (Doctorado). Según los datos, las variables que disminuirían la probabilidad de ocurrencia del evento (votar a Trump) son, sobre todo, que el género sea femenino u otro y que las personas tengan formación universitaria (ya que para ambos casos OR\<1).
